...
int check_for_jpeg(rust_input_handle_t handle)
{
  ...
  fprintf(stderr, "[jpeg] before seek\n");

  ttstub_input_seek(handle, 0, SEEK_SET);

  fprintf(stderr, "[jpeg] after seek, reading signature\n");

  if (ttstub_input_read(handle, (char *)jpeg_sig, 2) != 2)
  {
    fprintf(stderr, "[jpeg] signature read failed\n");
    return 0;
  }

  fprintf(stderr, "[jpeg] signature = %02x %02x\n", jpeg_sig[0], jpeg_sig[1]);
...
int jpeg_include_image(pdf_ximage *ximage, rust_input_handle_t handle)
{
  ...
  switch (j_info.num_components)
  {
    ...
    case 4:
      ...
      break;
    default:
      dpx_warning("%s: Unknown color space (num components: %d)",
  ...
  if (pdf_check_version(1, 4) >= 0)
  {
    if (j_info.flags & HAVE_APPn_XMP)
    {
      ...
      pdf_add_dict(stream_dict, pdf_new_name("Metadata"),
  ...
  if (j_info.flags & HAVE_APPn_ICC)
  {
    ...
    {
      if (iccp_check_colorspace(colortype, pdf_stream_dataptr(icc_stream),
...
#define IS_ADOBE_CMYK(j) \
  (((j).flags & HAVE_APPn_ADOBE) && (j).num_components == 4)
...
static void JPEG_info_clear(struct JPEG_info *j_info)
{
  if (j_info->num_appn > 0 && j_info->appn != NULL)
  {
    ...
    for (i = 0; i < j_info->num_appn; i++)
      JPEG_release_APPn_data(j_info->appn[i].marker, j_info->appn[i].app_sig,
                             j_info->appn[i].app_data);
    free(j_info->appn);
  }
  j_info->appn = NULL;
  j_info->num_appn = 0;
...
static pdf_obj *JPEG_get_iccp(struct JPEG_info *j_info)
{
  ...
  for (i = 0; i < j_info->num_appn; i++)
  {
    ...
    else if (icc->seq_id != prev_id + 1 || num_icc_seg != icc->num_chunks ||
             icc->seq_id > icc->num_chunks)
    {
      dpx_warning("Invalid JPEG ICC chunk: %d (p:%d, n:%d)", icc->seq_id,
                  prev_id, icc->num_chunks);
      pdf_release_obj(icc_stream);
      icc_stream = NULL;
      break;
    }
...
static pdf_obj *JPEG_get_XMP(struct JPEG_info *j_info)
{
  ...
  pdf_add_dict(stream_dict, pdf_new_name("Type"), pdf_new_name("Metadata"));
  pdf_add_dict(stream_dict, pdf_new_name("Subtype"), pdf_new_name("XML"));
  for (i = 0; i < j_info->num_appn; i++)
  {
  ...
  if (count > 1)
    dpx_warning("%s: Multiple XMP segments found in JPEG file. (untested)",
                JPEG_DEBUG_STR);

  return XMP_stream;
}

static JPEG_marker JPEG_get_marker(rust_input_handle_t handle)
{
  int c;
  int pos;
  ...
  if (c != 255)
  {
    fprintf(stderr, "[jpeg-marker] first byte=%02x\n", c & 0xff);
    return -1;
  }
  ...
  for (;;)
  {
    ...
    fprintf(stderr, "[jpeg-marker] byte at +%d = %02x\n", pos, c & 0xff);
...
static int read_exif_bytes(unsigned char **pp, int n, int endian)
{
  int rval = 0;
  unsigned char *p = *pp;
  int i;
...
static size_t read_APP1_Exif(struct JPEG_info *info,
                             rust_input_handle_t handle,
                             size_t length)
{
  ...
  while (num_fields-- > 0)
  {
    ...
    switch (tag)
    {
      ...
      case JPEG_EXIF_TAG_RESUNIT_MS: /* PixelUnit */
        ...
        {
          dpx_warning("%s: Invalid data for ResolutionUnit in Exif chunk.",
                      JPEG_DEBUG_STR);
          goto err;
        }
      ...
      case JPEG_EXIF_TAG_XRES_MS: /* PixelPerUnitX */
        ...
        {
          dpx_warning("%s: Invalid data for PixelPerUnitX in Exif chunk.",
                      JPEG_DEBUG_STR);
          goto err;
        }
        value = read_exif_bytes(&p, 4, bigendian);
      ...
      case JPEG_EXIF_TAG_YRES_MS: /* PixelPerUnitY */
        if (type != JPEG_EXIF_TYPE_LONG || count != 1)
        {
          dpx_warning("%s: Invalid data for PixelPerUnitY in Exif chunk.",
                      JPEG_DEBUG_STR);
          goto err;
        }
        value = read_exif_bytes(&p, 4, bigendian);
  ...
  else
  {
    ...
    {
      dpx_warning(
          "JPEG: Inconsistent resolution may have been "
...
static size_t read_APP0_JFIF(struct JPEG_info *j_info,
                             rust_input_handle_t handle)
{
...
static size_t read_APP0_JFXX(rust_input_handle_t handle, size_t length)
{
...
static size_t read_APP1_XMP(struct JPEG_info *j_info,
                            rust_input_handle_t handle,
                            size_t length)
{
...
static size_t read_APP2_ICC(struct JPEG_info *j_info,
                            rust_input_handle_t handle,
                            size_t length)
{
...
#define COPY_CHUNK(f, s, l)                                              \
  while ((l) > 0)                                                        \
  {                                                                      \
    int nb_read =                                                        \
        ttstub_input_read((f), work_buffer, MIN((l), WORK_BUFFER_SIZE)); \
    if (nb_read > 0)                                                     \
...

  {
    ...
    while ((length = ttstub_input_read(
                handle, work_buffer, MIN(WORK_BUFFER_SIZE, total_size - pos))) >
           0)
    {
...
static int JPEG_scan_file(struct JPEG_info *j_info, rust_input_handle_t handle)
{
  ...
  count = 0;
  found_SOFn = 0;
  while (!found_SOFn && (marker = JPEG_get_marker(handle)) != (JPEG_marker)-1)
  {
    if (marker != JM_SOI && (marker < JM_RST0 || marker > JM_RST7))
    {
...
int jpeg_get_bbox(rust_input_handle_t handle,
                  unsigned int *width,
                  unsigned int *height,
                  double *xdensity,
                  double *ydensity)
